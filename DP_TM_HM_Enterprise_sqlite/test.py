...
def get_data(user_id):
    query_string = "SELECT d.id AS dpth_id, d.device_name, d.dp_value, d.temp_value, d.hum_value, d.timestamp AS dpth_timestamp, dv.id AS device_id, dv.user_id FROM dpth_readings d JOIN (SELECT MAX(id) AS max_id, device_name FROM dpth_readings GROUP BY device_name) AS x ON d.id = x.max_id AND d.device_name = x.device_name JOIN device dv ON d.device_name = dv.device_name AND dv.user_id = ? ;"
...
def register():
        if request.method == 'POST':
            ...
            cursor.execute("SELECT * FROM enterprise WHERE username = ? or email = ? or mobile = ?", (username, email, mobile))
            existing_user = cursor.fetchall()
            if existing_user:
                error_message = "User already exists."
                return render_template('register.html', error=error_message)
              # Insert data into the database
            sql = "INSERT INTO enterprise (enterprisename, username, password, email, mobile, logopath, admin, datecreated) VALUES (?, ?, ?, ?, ?, ?, ?, ?)"
...
def login():
    ...
    if request.method == 'POST':
        ...
        password = request.form['password']
        print(password)
        cursor.execute('SELECT * FROM enterprise WHERE username = ? AND password = ? ;', (username, password))
...
def add_device():
     ...
     if request.method == 'POST':
        ...
        cursor.execute('INSERT INTO device (user_id, device_name, timestamp) VALUES (?, ?, ?);', (user_id, devicename, time))
...
def delete_device(): 
        ...
        # print("new", new)
        if not devices:
        # If there are no devices associated with the user, redirect or render a different template
            return render_template('temp_hum_dp.html', img=img_path)
        return render_template('delete_device.html' ,new = devices, img = img_path)

@app.route('/delete/<string:id>', methods=['GET', 'POST'])
def delete(id):
    cursor.execute("UPDATE device SET user_id = 1 where id = ?;", (id,))
    conn.commit()
    return redirect(url_for('home'))
