...
function display(data1) {
    ...
    function updateBackgroundColor() {
        ...
        data1.forEach((item, index) => {
            ...
            var formattedTimestamp = timestamp.toDateString() + ' ' + timestamp.toLocaleTimeString();
            ...
            if (diff_minutes(currentTime, timestamp)) {
                ...
                document.getElementById(`devicename_${index + 1}`).innerHTML = `${item.name} <span style="font-size: 12px; margin-left: 45px; color: black;">(offline)${formattedTimestamp}</span>`;
